...
static struct br_tv_template_entry devicePixelmapTemplateEntries[] = {
	{BRT_IDENTIFIER_CSTR,		F(pm_identifier),	BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, },
	{BRT_WIDTH_I32,				F(pm_width),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_I32_U16, },
	{BRT_HEIGHT_I32,			F(pm_height),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_I32_U16, },
	{BRT_PIXEL_TYPE_U8,			F(pm_type),			BRTV_QUERY | BRTV_ALL,	BRTV_CONV_I32_U8, },
//	{BRT_PIXEL_CHANNELS_I32,	0,					BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
//	{BRT_PIXEL_CHANNELS_TL,		0,					BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
//	{BRT_INDEXED_B,				F(pm_indexed),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
//	{BRT_ORIGIN_V2_I,			F(pm_indexed),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
//	{BRT_PIXEL_BITS_I32,		F(pm_indexed),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
//	{BRT_WORD_BYTES_I32,		F(pm_indexed),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
//	{BRT_MEMORY_MAPPED,			F(pm_indexed),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM, },
};
#undef F
...
static br_size_t BR_CMETHOD_DECL(br_device_pixelmap_mem, space)(br_device_pixelmap *self)
{
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_mem, validSource)
	(br_device_pixelmap *self, br_boolean *bp, br_object *h)
{
	/* ... */
	*bp = BR_FALSE;
	return BRE_OK;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_mem, copyTo)(br_device_pixelmap *self, br_device_pixelmap *src)
{
	...
	if (self->pm_row_bytes == src->pm_row_bytes &&
		...
		if(self->pm_row_bytes > 0)
			...
			_MemCopy_A(
				DevicePixelmapMemAddress(self,0,0,bytes),
				DevicePixelmapMemAddress(src,0,0,bytes),
				self->pm_width * self->pm_height, bytes);
		else
			_MemCopy_A(
				DevicePixelmapMemAddress(self,0,self->pm_height-1,bytes),
				DevicePixelmapMemAddress(src,0,src->pm_height-1,bytes),
				self->pm_width * self->pm_height, bytes);
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_mem, copyFrom)(br_device_pixelmap *self, br_device_pixelmap *dest)
{
	...
	if (self->pm_row_bytes == dest->pm_row_bytes &&
		...
		if(self->pm_row_bytes > 0)
			...
			_MemCopy_A(
				DevicePixelmapMemAddress(dest,0,0,bytes),
				DevicePixelmapMemAddress(self,0,0,bytes),
				self->pm_width * self->pm_height, bytes);
		else
			_MemCopy_A(
				DevicePixelmapMemAddress(dest,0,dest->pm_height-1,bytes),
				DevicePixelmapMemAddress(self,0,self->pm_height-1,bytes),
				self->pm_width * self->pm_height, bytes);
	...
	(br_device_pixelmap *self, br_point *p, br_device_pixelmap *src, br_rectangle *r)
{
	...
	if (ar.w == self->pm_width && self->pm_row_bytes == src->pm_row_bytes &&
		...
		if(self->pm_row_bytes > 0)
			...
			_MemCopy_A(
				DevicePixelmapMemAddress(self,ap.x,ap.y,bytes),
				DevicePixelmapMemAddress(src,ar.x,ar.y,bytes),
				ar.w * ar.h,bytes);
		else
			_MemCopy_A(
				DevicePixelmapMemAddress(self,ap.x,ap.y+ar.h-1,bytes),
				DevicePixelmapMemAddress(src,ar.x,ar.y+ar.h-1,bytes),
				ar.w * ar.h,bytes);
	...
	(br_device_pixelmap *self, br_point *p, br_device_pixelmap *dest, br_rectangle *r)
{
	...
	if (ar.w == self->pm_width && self->pm_row_bytes == dest->pm_row_bytes &&
		...
		if(self->pm_row_bytes > 0)
			...
			_MemCopy_A(
				DevicePixelmapMemAddress(dest,ap.x,ap.y,bytes),
				DevicePixelmapMemAddress(self,ar.x,ar.y,bytes),
				ar.w * ar.h,bytes);
		else
			_MemCopy_A(
				DevicePixelmapMemAddress(dest,ap.x,ap.y+ar.h-1,bytes),
				DevicePixelmapMemAddress(self,ar.x,ar.y+ar.h-1,bytes),
				ar.w * ar.h,bytes);
	...
	(br_device_pixelmap *self, br_point *p, br_uint_32 colour)
{
	...
	_MemPixelSet(DevicePixelmapMemAddress(self,ap.x,ap.y,bytes),
		bytes, colour);
