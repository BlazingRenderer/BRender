...
static struct br_tv_template_entry templateEntries[] = {
	...
	{BRT(OPENGL_MSAA_SAMPLES_I32),	F(msaa_samples),	BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY,}
};
#undef F
...
{
	{BRT(OPENGL_MSAA_SAMPLES_I32),	F(msaaSamples),	BRTV_SET,	BRTV_CONV_COPY,}
};
#undef F
...
#define F(f) offsetof(struct matchTokens, f)
static struct br_tv_template_entry matchTemplateEntries[] = {
	{BRT_WIDTH_I32,			F(width),		BRTV_SET,	BRTV_CONV_COPY,},
	{BRT_HEIGHT_I32,		F(height),		BRTV_SET,	BRTV_CONV_COPY,},
	{BRT_PIXEL_BITS_I32,	F(pixel_bits),	BRTV_SET,	BRTV_CONV_COPY,},
	{BRT_USE_T,				F(use_type),	BRTV_SET,	BRTV_CONV_COPY,},
};
#undef F


static void _patchQuad(br_device_pixelmap_gl_quad *self, float dx0, float dy0, float dx1, float dy1, float sx0, float sy0, float sx1, float sy1)
{
	/* Bottom-left */
	self->tris[0].x = dx0;
	self->tris[0].y = dy0;
...
static void patchQuad(br_device_pixelmap_gl *self, br_rectangle *dr, br_pixelmap *src, br_rectangle *sr)
{
	...
	_patchQuad(&self->asBack.quad, x0, y0, x1, y1, sx0, sy0, sx1, sy1);
}
...
static br_error initPixelmapGL(br_device_pixelmap_gl *self)
{
	...
	else if(self->use_type == BRT_DEPTH)
	{
		...
		if(self->msaa_samples)
		{
			glRenderbufferStorageMultisample(GL_RENDERBUFFER, self->msaa_samples, GL_DEPTH_COMPONENT24, self->pm_width, self->pm_height);
			checkGlError("glRenderbufferStorageMultisample");
		}
		else
		{
...
void br_device_pixelmap_gl_free(br_object *_self)
{
	...
	ObjectContainerRemove(self->output_facility, self);

	OutputFacilityOnPixelmapFree(self->output_facility, self);
}
...
br_size_t br_device_pixelmap_gl_space(br_object *self)
{
...
{
	br_device_pixelmap_gl *self = reinterpret_cast<br_device_pixelmap_gl*>(_self);
	if(self->device->templates.devicePixelmap == nullptr)
		self->device->templates.devicePixelmap = BrTVTemplateAllocate(self->device, templateEntries, BR_ASIZE(templateEntries));
	return self->device->templates.devicePixelmap;
}
...
br_error br_device_pixelmap_gl_match(br_device_pixelmap *_self, br_device_pixelmap **_newpm, br_token_value *tv)
{
	...
	if(self->device->templates.pixelmapMatch == nullptr)
		self->device->templates.pixelmapMatch = BrTVTemplateAllocate(self->device, matchTemplateEntries, BR_ASIZE(matchTemplateEntries));

	br_int_32 count;
	BrTokenValueSetMany(&mt, &count, nullptr, tv, self->device->templates.pixelmapMatch);
	...
	br_device_pixelmap_gl *newpm = BrResAllocate<br_device_pixelmap_gl>(self->output_facility, BR_MEMORY_OBJECT);

	newpm->dispatch = self->dispatch;
	newpm->device = self->device;
	newpm->output_facility = self->output_facility;
	newpm->use_type = mt.use_type;
	newpm->msaa_samples = self->msaa_samples;

	newpm->pm_type = self->pm_type;
	newpm->pm_width = static_cast<br_uint_16>(mt.width);
	newpm->pm_height = static_cast<br_uint_16>(mt.height);
	...
	ObjectContainerAddFront(self->output_facility, newpm);
...
static const struct br_device_pixelmap_dispatch devicePixelmapDispatch = {
	...
	.__reserved2				= nullptr,
	.__reserved3				= nullptr,
	._free						= br_device_pixelmap_gl_free,
	._identifier				= br_device_pixelmap_gl_identifier,
	._type						= br_device_pixelmap_gl_type,
	._isType					= br_device_pixelmap_gl_isType,
	._device					= br_device_pixelmap_gl_device,
	._space						= br_device_pixelmap_gl_space,

	._templateQuery				= br_device_pixelmap_gl_templateQuery,
	...
	._validSource				= br_device_pixelmap_gl_validSource,
	._resize					= br_device_pixelmap_gl_resize,
	._match						= br_device_pixelmap_gl_match,
	._allocateSub				= br_device_pixelmap_gl_allocateSub,

	._copy						= BR_CMETHOD(br_device_pixelmap_gen, copy),
	._copyTo					= BR_CMETHOD(br_device_pixelmap_gen, copyTo),
	._copyFrom					= BR_CMETHOD(br_device_pixelmap_gen, copyFrom),
	._fill						= BR_CMETHOD(br_device_pixelmap_gen, fill),
	._doubleBuffer				= br_device_pixelmap_gl_doubleBuffer,

	._copyDirty					= BR_CMETHOD(br_device_pixelmap_gen, copyDirty),
	._copyToDirty				= BR_CMETHOD(br_device_pixelmap_gen, copyToDirty),
	._copyFromDirty				= BR_CMETHOD(br_device_pixelmap_gen, copyFromDirty),
	._fillDirty					= BR_CMETHOD(br_device_pixelmap_gen, fillDirty),
	._doubleBufferDirty			= BR_CMETHOD(br_device_pixelmap_gen, doubleBufferDirty),

	._rectangle					= BR_CMETHOD(br_device_pixelmap_gen, rectangle),
	._rectangle2				= BR_CMETHOD(br_device_pixelmap_gen, rectangle2),
	._rectangleCopy				= br_device_pixelmap_gl_rectangleCopy,
	._rectangleCopyTo			= br_device_pixelmap_gl_rectangleCopyTo,
	._rectangleCopyFrom			= br_device_pixelmap_gl_rectangleCopyFrom,
	._rectangleStretchCopy		= br_device_pixelmap_gl_rectangleStretchCopy,
	._rectangleStretchCopyTo	= br_device_pixelmap_gl_rectangleStretchCopyTo,
	._rectangleStretchCopyFrom	= br_device_pixelmap_gl_rectangleStretchCopyFrom,
	._rectangleFill				= br_device_pixelmap_gl_rectangleFill,
	._pixelSet					= br_device_pixelmap_lock_gl_pixelSet,
	._line						= br_device_pixelmap_lock_gl_line,
	._copyBits					= br_device_pixelmap_lock_gl_copyBits,

	._text						= br_device_pixelmap_gl_text,
	._textBounds				= BR_CMETHOD(br_device_pixelmap_gen, textBounds),

	._rowSize					= br_device_pixelmap_fail_gl_rowSize,
	._rowSet					= br_device_pixelmap_fail_gl_rowSet,
	._rowQuery					= br_device_pixelmap_fail_gl_rowQuery,

	._pixelQuery				= br_device_pixelmap_lock_gl_pixelQuery,
	._pixelAddressQuery			= br_device_pixelmap_fail_gl_pixelAddressQuery,

	._pixelAddressSet			= br_device_pixelmap_fail_gl_pixelAddressSet,
	._originSet					= br_device_pixelmap_gl_originSet,

	._flush						= br_device_pixelmap_gl_flush,
	._synchronise				= br_device_pixelmap_gl_synchronise,
	._directLock				= br_device_pixelmap_gl_directLock,
	._directUnlock				= br_device_pixelmap_gl_directUnlock,
};

br_device_pixelmap_gl *DevicePixelmapGLAllocate(br_device_gl *device, br_output_facility_gl *facility, br_token_value *tv)
{
	struct pixelmapNewTokens pt = { .msaaSamples = 0 };
	br_int_32 count;

	if(device->templates.pixelmapNew == nullptr)
		device->templates.pixelmapNew = BrTVTemplateAllocate(device, pixelmapNewTemplateEntries, BR_ASIZE(pixelmapNewTemplateEntries));

	BrTokenValueSetMany(&pt, &count, nullptr, tv, device->templates.pixelmapNew);

	if(pt.msaaSamples < 0)
		pt.msaaSamples = 0;
	else if(pt.msaaSamples > device->video.maxSamples)
		pt.msaaSamples = device->video.maxSamples;

	br_device_pixelmap_gl *self = BrResAllocate<br_device_pixelmap_gl>(facility, BR_MEMORY_OBJECT);
	...
	self->msaa_samples = pt.msaaSamples;
