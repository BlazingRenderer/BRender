...
static void renderByMode(br_renderer_gl *self, render_mode mode, bool depthOverride)
{
	for(auto& r : self->geom)
	{
...
br_error br_geometry_v1_buckets_gl_render(struct br_geometry_v1_buckets *_self, struct br_renderer *_renderer, struct br_primitive **buckets, br_int_32 nbuckets)
{
	...
	renderByMode(renderer, render_mode::force_back, true);
	renderByMode(renderer, render_mode::opaque, false);
	renderByMode(renderer, render_mode::transparent, false);
	renderByMode(renderer, render_mode::force_front, true);
