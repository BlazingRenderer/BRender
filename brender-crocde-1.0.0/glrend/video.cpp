...
bool VIDEOI_CompileShaders(HVIDEO hVideo)
{
	...
	if(!VIDEOI_CompileBRenderShader(hVideo))
	{
...
extern "C" VIDEO_API HVIDEO VIDEO_Open(HVIDEO hVideo)
{
	if(hVideo == nullptr)
	{
	...
	strncpy(hVideo->version, reinterpret_cast<const char *>(glGetString(GL_VERSION)), sizeof(hVideo->version));
	hVideo->version[sizeof(hVideo->version) - 1] = '\0';
	strncpy(hVideo->vendor, reinterpret_cast<const char *>(glGetString(GL_VENDOR)), sizeof(hVideo->vendor));
	hVideo->vendor[sizeof(hVideo->vendor) - 1] = '\0';
	strncpy(hVideo->renderer, reinterpret_cast<const char *>(glGetString(GL_RENDERER)), sizeof(hVideo->renderer));
	hVideo->renderer[sizeof(hVideo->renderer) - 1] = '\0';
	...
	if(!VIDEOI_CompileShaders(hVideo))
		return nullptr;
...
GLchar *VIDEOI_LoadFileFromDisk(const char *name, size_t *size)
{
	int currentSize = 64;
	...
	for(;; currentSize += 64)
	{
		...
		if(!(brf = BrFileOpenRead(name, 0, nullptr, nullptr)))
			return nullptr;

		if(!(buffer = reinterpret_cast<GLchar*>(BrScratchAllocate(currentSize))))
		{
			BrFileClose(brf);
			return nullptr;
		}
