...
br_device_pixelmap *DevicePixelmapDirectDrawAllocate(br_device *dev, br_output_facility *facility, br_token_value *tv)
{
    ...
    if(dev->templates.hwndTemplate == NULL) {
        dev->templates.hwndTemplate = BrTVTemplateAllocate(dev->res, hwndTemplateEntries, BR_ASIZE(hwndTemplateEntries));
    }
    ...
    if(self->fullscreen) {
        ...
        if(IDirectDraw2_SetDisplayMode(self->DD, width, height, bpp, 0,
                                       (facility->standard_vga ? DDSDM_STANDARDVGAMODE : 0)) != DD_OK) {
#else
    ...
    switch(self->pm_type) {
        case BR_PMT_INDEX_8:
            ...
            IDirectDraw2_CreatePalette(self->DD, DDPCAPS_8BIT | (facility->fullscreen ? DDPCAPS_ALLOW256 : 0), entries,
                                       &self->palette, NULL);
...
br_device *BR_CMETHOD_DECL(br_device_pixelmap_dd, device)(br_object *self)
{
 ...
 */
static br_error BR_CMETHOD_DECL(br_device_pixelmap_dd, resize)(br_device_pixelmap *self, br_int_32 width, br_int_32 height)
{
    br_uint_16    bpp;
    DDSURFACEDESC ddsd;
 ...
 */
static void BR_CMETHOD_DECL(br_device_pixelmap_dd, free)(br_object *_self)
{
 ...
 */
static br_error BR_CMETHOD_DECL(br_device_pixelmap_dd, match)(br_device_pixelmap *self, br_device_pixelmap **newpm,
                                                              br_token_value *tv)
{
    br_int_32           count;
    ...
    if(self->device->templates.matchTemplate == NULL) {
        self->device->templates.matchTemplate = BrTVTemplateAllocate(self->device->res, matchTemplateEntries,
 ...
 */
static br_error BR_CMETHOD_DECL(br_device_pixelmap_dd, doubleBuffer)(br_device_pixelmap *self, br_device_pixelmap *src)
{
    POINT pt;
    RECT  srect, drect;
    ...
    while(1) {
        ...
        switch(IDirectDrawSurface_Blt(self->output_facility->modex ? self->offscreen : self->surface, &drect,
                                      src->surface, &srect, DDBLT_WAIT, NULL)) {
			...
			if (IDirectDrawSurface_Restore(self->output_facility->modex? self->offscreen: self->surface) != DD_OK ||
                ...
                if(IDirectDrawSurface_Restore(self->surface) != DD_OK || IDirectDrawSurface_Restore(src->surface) != DD_OK)
#endif
...
struct br_tv_template *BR_CMETHOD_DECL(br_device_pixelmap_dd, queryTemplate)(br_object *_self)
{
    br_device_pixelmap *self = (br_device_pixelmap *)_self;

    if(self->device->templates.devicePixelmapTemplate == NULL)
        self->device->templates.devicePixelmapTemplate = BrTVTemplateAllocate(
            self->device->res, devicePixelmapTemplateEntries, BR_ASIZE(devicePixelmapTemplateEntries));
 ...
 */
br_error BR_CMETHOD_DECL(br_device_pixelmap_dd, allocateSub)(br_device_pixelmap *self, br_device_pixelmap **newpm,
                                                             br_rectangle *rect)
{
    (void)self;
    (void)newpm;
...
static struct br_device_pixelmap_dispatch devicePixelmapDispatch = {
    ...
    .__reserved2 = NULL,
    .__reserved3 = NULL,
    ._free       = BR_CMETHOD_REF(br_device_pixelmap_dd, free),
    ...
    ._device     = BR_CMETHOD_REF(br_device_pixelmap_dd, device),
    ...
    ._resize      = BR_CMETHOD_REF(br_device_pixelmap_dd, resize),
    ._match       = BR_CMETHOD_REF(br_device_pixelmap_dd, match),
    ._allocateSub = BR_CMETHOD_REF(br_device_pixelmap_dd, allocateSub),
    ...
    ._doubleBuffer = BR_CMETHOD_REF(br_device_pixelmap_dd, doubleBuffer),
