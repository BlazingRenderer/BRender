...
static struct br_tv_template devicePixelmapTemplate = {BR_ASIZE(devicePixelmapTemplateEntries), devicePixelmapTemplateEntries};
...
br_device_pixelmap *DevicePixelmapVESAAllocateMode(br_device *dev, br_output_facility *facility, br_uint_16 w, br_uint_16 h)
{
     ...
     */
    BrSprintfN(tmp, sizeof(tmp) - 1, "%dx%dx%d %s %s", self->pm_width, self->pm_height, OutputFacilityVESABits(facility),
               vw->access_linear ? "Linear" : (vw->scanline_breaks ? "Banked-Breaks" : "Banked"), vw->bank_protected ? "Protected" : "Int10");
...
struct br_tv_template *BR_CMETHOD_DECL(br_device_pixelmap_vesa, templateQuery)(br_device_pixelmap *self)
{
    devicePixelmapTemplate.res = DeviceVESAResource(ObjectVESADevice(self));
    return &devicePixelmapTemplate;
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_fix, flush)(struct br_device_pixelmap *self)
{
    return BRE_OK;
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_fix, synchronise)(struct br_device_pixelmap *self, br_token sync_type, br_boolean block)
{
    return BRE_OK;
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_fix, directLock)(struct br_device_pixelmap *self, br_boolean block)
{
    return BRE_OK;
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_fix, directUnlock)(struct br_device_pixelmap *self)
{
    return BRE_OK;
}
