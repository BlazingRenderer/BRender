...
static struct br_tv_template_entry device2TemplateEntries[] = {
     ...
     BRTV_CONV_I32_U16, },
    {
     ...
     BRTV_CONV_COPY, },
    {
     DEV(OEM_PRODUCT_NAME_CSTR),
     F(info.oem_product_name_ptr),
     BRTV_QUERY | BRTV_ALL,
     BRTV_CONV_COPY, },
    {
     DEV(OEM_PRODUCT_REV_CSTR),
     F(info.oem_product_rev_ptr),
     BRTV_QUERY | BRTV_ALL,
     BRTV_CONV_COPY, },
    {
     DEV(OEM_VENDOR_NAME_CSTR),
     F(info.oem_vendor_name_ptr),
     BRTV_QUERY | BRTV_ALL,
     BRTV_CONV_COPY, },
    {
     BRT(CLUT_O),
...
static struct br_tv_template device1Template = {BR_ASIZE(device1TemplateEntries), device1TemplateEntries};

static struct br_tv_template device2Template = {BR_ASIZE(device2TemplateEntries), device2TemplateEntries};
...
static struct br_tv_template deviceArgsTemplate = {BR_ASIZE(deviceArgsTemplateEntries), deviceArgsTemplateEntries};
...
br_error DeviceVESABankSwitchInitialise(br_device *self)
{
     ...
     * Allocate a buffer
     */
    buffer = BrResAllocate(DeviceVESAResource(self), size, BR_MEMORY_DRIVER);
...
br_error DeviceVESABankedInitialise(br_device *self, struct vesa_modeinfo *mi)
{
    ...
    if(mi->y_resolution > vw->scanline_max) {
        ...
        vw->scanline_table = BrResAllocate(DeviceVESAResource(self), vw->scanline_max * sizeof(struct vesa_scanline), BR_MEMORY_DRIVER);
    }
...
br_error DeviceVESALinearInitialise(br_device *self, struct vesa_modeinfo *mi)
{
    ...
    if(vw->physical_selector == 0) {
        ...
        r     = HostSelectorAllocatePhysical(&vw->physical_selector, mi->phys_base_ptr, psize);
...
static struct br_tv_template *BR_CMETHOD_DECL(br_device_vesa, queryTemplate)(br_device *self)
{
    if(self->info.vbe_version >= 0x200) {
...
static void *BR_CMETHOD_DECL(br_device_vesa, listQuery)(br_device *self)
{
