...
br_error BR_CMETHOD_DECL(br_device_pixelmap_vesa_wb, rectangleCopyTo)(br_device_pixelmap *self, br_point *p, br_device_pixelmap *src,
                                                                      br_rectangle *r)
{
    int          bytes;
    br_rectangle ar;
    ...
    VESAWindowBreaksRectangleCopyTo(DevicePixelmapMemAddress(src, ar.x, ar.y, bytes), src->pm_pixels_qualifier, src->pm_row_bytes, ap.x,
                                    ap.y, ar.w, ar.h);

    return BRE_OK;
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_vesa_wb, rectangleCopyFrom)(br_device_pixelmap *self, br_point *p, br_device_pixelmap *mem,
                                                                        br_rectangle *r)
{
    int          bytes;
    br_rectangle ar;
    ...
    VESAWindowBreaksRectangleCopyFrom(DevicePixelmapMemAddress(mem, ap.x, ap.y, bytes), mem->pm_pixels_qualifier, mem->pm_row_bytes, ar.x,
                                      ar.y, ar.w, ar.h);

    return BRE_OK;
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_vesa_wb, rectangleFill)(br_device_pixelmap *self, br_rectangle *rect, br_uint_32 colour)
{
    br_rectangle arect;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_vesa_wb, pixelSet)(br_device_pixelmap *self, br_point *p, br_uint_32 colour)
{
    br_point ap;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_vesa_wb, copyBits)(br_device_pixelmap *self, br_point *point, br_uint_8 *src,
                                                               br_uint_16 s_stride, br_rectangle *bit_rect, br_uint_32 colour)
{
    ...
    VESAWindowBreaksCopyBits(ap.x + (ar.x & ~7) - bit, ap.y, src + ar.y * s_stride, s_stride, bit, bit + ar.w, ar.h, colour);
