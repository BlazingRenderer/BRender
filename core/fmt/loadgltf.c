...
static cgltf_result cgltf_load_brfile(const struct cgltf_memory_options *memory_options, const struct cgltf_file_options *file_options,
...
static void cgltf_release_brfile(const struct cgltf_memory_options *memory_options, const struct cgltf_file_options *file_options, void *data)
{
    (void)memory_options;
    (void)file_options;
...
static br_pixelmap *qwerqwe(const cgltf_image *image)
{
    //    cgltf_decode_uri()
    return NULL;
}
...
static void fill_material(br_material *mat, const cgltf_brender_material *br_material)
{
...
br_fmt_results *BR_PUBLIC_ENTRY BrFmtGLTFActorLoadMany(const char *name)
{
    ...
    for(br_size_t i = 0; i < data->images_count; ++i) {
        results->pixelmaps[i] = qwerqwe(data->images + i);
    }
    ...
    for(br_size_t i = 0; i < data->brender_materials_count; ++i) {
        ...
        fill_material(results->materials[i], data->brender_materials + i);
    }
