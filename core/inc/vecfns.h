 ...
 * 4D Vectors
 */
#define BrVector4Set(v1, s1, s2, s3, s4) \
    do {                                 \
        (v1)->v[0] = (s1);               \
        (v1)->v[1] = (s2);               \
        (v1)->v[2] = (s3);               \
 ...
 * v1 = v2/|v2|
 */
#define BrVector4Normalise(v1, v2)                                           \
    do {                                                                     \
        br_scalar _scale;                                                    \
        _scale = BR_LENGTH4((v2)->v[0], (v2)->v[1], (v2)->v[2], (v2)->v[3]); \
        if(_scale > (BR_SCALAR_EPSILON * 2)) {                               \
            _scale     = BR_RCP(_scale);                                     \
            (v1)->v[0] = BR_MUL((v2)->v[0], _scale);                         \
            (v1)->v[1] = BR_MUL((v2)->v[1], _scale);                         \
            (v1)->v[2] = BR_MUL((v2)->v[2], _scale);                         \
            (v1)->v[3] = BR_MUL((v2)->v[3], _scale);                         \
        } else {                                                             \
            (v1)->v[0] = BR_SCALAR(1.0);                                     \
            (v1)->v[1] = BR_SCALAR(0.0);                                     \
            (v1)->v[2] = BR_SCALAR(0.0);                                     \
