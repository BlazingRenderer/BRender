...
br_uint_32 BR_PUBLIC_ENTRY BrZbScreenZToDepth(br_scalar sz, const br_camera *camera)
{
    ...
    if(sz <= TOONEAR)
        ...
        return 0xffffffffUL;
#else /* Alternative that caters for border line cases - if they can occur */
                                         ...
                                         br_int_16 y, br_scalar sz)
{
    ...
    hx = BR_DIV(BrIntToScalar(screen_buffer->width - 2 * (x + screen_buffer->origin_x)),
                BrIntToScalar(screen_buffer->width)); // homogenous -x	[ hx=2(sx+ox)/sw-1, -hx=(sw-2(sx+ox))/sw ]
    hy = BR_DIV(BrIntToScalar(2 * (y + screen_buffer->origin_y) - screen_buffer->height),
                BrIntToScalar(screen_buffer->height)); // homogenous -y	[ hy=1-2(sy+oy)/sh, -hy=(2(sy+oy)-sh)/sh ]
    ...
    switch(data->type) {
        ...
        case BR_CAMERA_PERSPECTIVE:
            ...
            scale = BR_DIV(BR_COS(fov),
                           BR_SIN(fov)); // Refer to BrMatrix4Perspective() - we are using its terms in reverse
