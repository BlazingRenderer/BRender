...
#if BRENDER_BUILD_FOR_CROC // Kludging flat shading for Croc
        /*
...
void BR_PUBLIC_ENTRY BrModelUpdate(br_model *model, br_uint_16 flags)
{
    ...
    if(!(model->flags & BR_MODF_CUSTOM_BOUNDS) && (flags & (BR_MODU_VERTICES | BR_MODU_VERTEX_POSITIONS))) {
        ...
        // use the average of the bounds as an approximation to the centre of the model (that gives the minimal bounding
        // radius)
