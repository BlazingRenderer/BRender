...
static void buildVbo(br_geometry_stored_gl *self, struct v11model_f *model, size_t totalVertices)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, self->glVbo);
	glBufferData(GL_ARRAY_BUFFER, totalVertices * sizeof(gl_vertex_f), nullptr, GL_STATIC_DRAW);
	gl_vertex_f *vtx = (gl_vertex_f*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
...
{
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self->glIbo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, totalFaces * 3 * sizeof(br_uint_16), nullptr, GL_STATIC_DRAW);

	br_uint_16 *idx = (br_uint_16*)glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY);
	...
	while(glUnmapBuffer(GL_ELEMENT_ARRAY_BUFFER) == GL_FALSE);
