...
bool VIDEOI_CompileBRenderShader(HVIDEO hVideo, const char *vertPath, const char *fragPath)
{
	...
	size_t size;
	GLchar *vertSource = reinterpret_cast<GLchar*>(BrFileLoad(nullptr, vertPath, &size));
	...
	if(!vert)
	{
		BrResFree(vertSource);
		return false;
	}

	BrResFree(vertSource);

	GLchar *fragSource = reinterpret_cast<GLchar*>(BrFileLoad(nullptr, fragPath, &size));
