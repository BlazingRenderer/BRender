...
static br_error BR_CMETHOD_DECL(br_geometry_v1_model_gl, storedNew)(struct br_geometry_v1_model *self,
                                                                    struct br_renderer *renderer,
                                                                    struct br_geometry_stored **psg,
                                                                    struct v11model_f *model,
                                                                    br_token type,
                                                                    br_token_value *tv)
{
    ASSERT(type == BRT_TRIANGLE);
    if((*psg = GeometryStoredGLAllocate(self, "MODEL", (struct v11model*)model)) == NULL)
        return BRE_FAIL;
