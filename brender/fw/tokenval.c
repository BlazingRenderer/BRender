...
static void DumpMatrixInteger(br_int_32 *ip, int rows, int cols,
	const char *prefix, char *info_0, char *info_n,
...
static void DumpObject(br_object *h,
	const char *prefix, const char *info,
	br_putline_cbfn *putline, void *arg)
{
	char value[128];
		...
		if(BrLexerCurrent(l) == T_COLON ||
			...
			case T_INTEGER: {
				static br_token pos_int_types[] = {
					BRT_INT_64, BRT_UINT_64, BRT_INT_32, BRT_UINT_32, BRT_INT_16, BRT_UINT_16,
					BRT_INT_8, BRT_UINT_8, BRT_FLOAT, BRT_FIXED, BRT_BOOLEAN,
					BRT_POINTER, BRT_OBJECT, BRT_HANDLE, BRT_INTPTR, BRT_UINTPTR};
...
br_value BR_RESIDENT_ENTRY BrTokenValueVaArg(br_token token, va_list ap)
{
    switch(BrTokenType(token)) {
        ...
        case BRT_INT_32:
            return (br_value){.i32 = va_arg(ap, br_int_32)};
        case BRT_UINT_32:
