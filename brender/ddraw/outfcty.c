...
br_error OutputFacilityDirectDrawInitialise(br_device *dev)
{
    ...
    if(IDirectDraw2_SetCooperativeLevel(dev->DD, dev->window,
                                        dev->window != NULL ? (DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN | DDSCL_ALLOWMODEX)
                                                            : DDSCL_NORMAL) == DD_OK) {
    ...
    if(num_vga_facilities != 0) {
        ...
        if(vga_facility != NULL)
            ...
            ObjectContainerFindMany(dev, (br_object **)vga_facility, num_vga_facilities, &num_vga_facilities,
                                    BRT_OUTPUT_FACILITY, NULL, tv);
    ...
    if(num_modex_facilities != 0) {
        ...
        if(modex_facility != NULL)
            ...
            ObjectContainerFindMany(dev, (br_object **)modex_facility, num_modex_facilities, &num_modex_facilities,
                                    BRT_OUTPUT_FACILITY, NULL, tv);
    ...
    if(num_linear_facilities != 0) {
        ...
        if(linear_facility != NULL)
            ...
            ObjectContainerFindMany(dev, (br_object **)linear_facility, num_linear_facilities, &num_linear_facilities,
                                    BRT_OUTPUT_FACILITY, NULL, tv);
    ...
    for(i = 0; i < num_modex_facilities; i++) {
        ...
        for(j = 0; j < num_vga_facilities; j++)
            ...
            if(modex_facility[i]->width == vga_facility[j]->width && modex_facility[i]->height == vga_facility[j]->height &&
    ...
    for(i = 0; i < num_vga_facilities; i++)
        ...
        for(j = 0; j < num_linear_facilities; j++)
            ...
            if(vga_facility[i]->width == linear_facility[j]->width && vga_facility[i]->height == linear_facility[j]->height &&
...
static struct br_tv_template *BR_CMETHOD_DECL(br_output_facility_dd, queryTemplate)(br_object *_self)
{
    ...
    if(self->device->templates.outputFacilityTemplate == NULL)
        self->device->templates.outputFacilityTemplate = BrTVTemplateAllocate(
            self->device->res, outputFacilityTemplateEntries, BR_ASIZE(outputFacilityTemplateEntries));
 ...
 */
static br_error BR_CMETHOD_DECL(br_output_facility_dd, pixelmapNew)(br_output_facility  *self,
 ...
 */
static br_error BR_CMETHOD_DECL(br_output_facility_dd, clutNew)(br_output_facility *self, br_device_clut **pclut,
                                                                br_token_value *tv)
{
    return BRE_FAIL;
}
...
static br_error BR_CMETHOD_DECL(br_output_facility_dd,
                                queryCapability)(br_output_facility *self, br_token_value *buffer_in,
