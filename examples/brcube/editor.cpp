...
Editor::Editor::Editor()
    : m_screen(nullptr), m_log_handler(), m_log_pane("LogPane"), m_about_dialog("AboutDialog"),
      m_next_camera_pane_no(0), m_world(nullptr), m_graph_pane(0)
{
    ...
    if(ImGui::BeginMainMenuBar()) {
        if(ImGui::BeginMenu("File")) {
            if(ImGui::MenuItem("Exit")) {
                SDL_Event evt{
                    .quit = (SDL_QuitEvent){.type = SDL_QUIT, .timestamp = SDL_GetTicks()},
                };
