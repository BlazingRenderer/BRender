...
Editor::AboutDialog::AboutDialog(const char *id)
    : m_screen(nullptr), m_visible(false), m_gl_buffer(), m_gl_extensions(nullptr), m_gl_num_extensions(0),
      m_gl_current_extension(0)
{
...
void Editor::AboutDialog::DrawHeader()
{
    ...
    ImGui::BeginChild("Header", ImVec2(content_region.x, this->m_pm_logo->height), ImGuiChildFlags_None);
    {
        ...
        if(logo_tex != 0) {
            ImGui::Image(reinterpret_cast<ImTextureID>(logo_tex),
                         ImVec2(this->m_pm_logo->width, this->m_pm_logo->height), ImVec2(0, 0), ImVec2(1, 1));
        }
        ImGui::EndChild();
        ...
        {
            ImGui::Text("BReditor");
            ImGui::Text("using BRender %d.%d.%d", (__BRENDER__ & 0xff0000) >> 16, (__BRENDER__ & 0x00ff00) >> 8,
    ...
    if(ImGui::BeginPopupModal(this->m_id.get(), nullptr, ImGuiWindowFlags_NoResize)) {
        ...
        if(ImGui::BeginTabBar("XXXXX", ImGuiTabBarFlags_None)) {
            ...
            if(ImGui::BeginTabItem("OpenGL")) {
                ...
                ImGui::InputText("Version", this->m_gl_version.data(), this->m_gl_version.size(), ImGuiInputTextFlags_ReadOnly);
                ImGui::InputText("Renderer", this->m_gl_renderer.data(), this->m_gl_renderer.size(),
                                 ImGuiInputTextFlags_ReadOnly);
                ImGui::InputText("Vendor", this->m_gl_vendor.data(), this->m_gl_vendor.size(), ImGuiInputTextFlags_ReadOnly);
