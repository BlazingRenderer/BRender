...
static void BR_CALLBACK RenderBoundsCallback(br_actor *actor, br_model *model, br_material *material, void *render_data,
                                             br_uint_8 style, br_matrix4 *model_to_screen, br_int_32 bounds[4]);
static void(BR_CALLBACK *oldrendercallback)(br_actor *actor, br_model *model, br_material *material, void *render_data,
                                            br_uint_8 style, br_matrix4 *model_to_screen, br_int_32 bounds[4]);
    ...
    for(anim_loop = 0; anim_loop < 10; anim_loop++) {
        ...
        for(i = 0, model_number = 0; i < intro_num_models; i++) {
            BrPixelmapRectangleFill(colour_buffer, render_bounds[i][0] - 1, render_bounds[i][1] - 1,
                                    ...
                                    render_bounds[i][3] - render_bounds[i][1] + 2, background_colour);

            BrPixelmapRectangleFill(z_buffer, render_bounds[i][0] - 1, render_bounds[i][1] - 1,
                                    render_bounds[i][2] - render_bounds[i][0] + 2,
                                    render_bounds[i][3] - render_bounds[i][1] + 2, 0xffffffff);
        }
