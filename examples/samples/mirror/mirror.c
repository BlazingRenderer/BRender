...
static void BR_CALLBACK ViewBoundsCallback(br_actor *actor, br_model *model, br_material *material, void *render_data, br_uint_8 style,
                                           br_matrix4 *model_to_screen, br_int_32 *bounds);

void ControlObject(br_actor *camera_actor, br_actor *translate_actor, br_actor *rotate_actor, br_int_32 mouse_x, br_int_32 mouse_y);
...
br_error BrDemo_InitApp(int argc, char **argv)
{
    ...
    r = BrDemo_Allocate_Pixelmaps("Mirror Demo", &screen, &offscreen, &depth, &palette, "winstd.pal");
...
br_error BrDemo_Render_Loop()
{
    ...
    if(offscreen->type != BR_PMT_INDEX_8) {
        BrPixelmapTextF(offscreen, -(BrPixelmapTextWidth(offscreen, BrFontProp7x9, "ONLY 8 BIT SCREEN MODES ARE SUPPORTED") / 2),
                        (BrFontProp7x9->glyph_y) / 2, BR_COLOUR_RGB(200, 200, 200), BrFontProp7x9, "ONLY 8 BIT SCREEN MODES ARE SUPPORTED");
...
void ControlObject(br_actor *camera_actor, br_actor *translate_actor, br_actor *rotate_actor, br_int_32 mouse_x, br_int_32 mouse_y)
{
    ...
    if((mouse_buttons & BR_MSM_BUTTONL) && (mouse_buttons & BR_MSM_BUTTONR)) {
