...
void displayhelp(br_pixelmap *buffer, int x, int y, int colour)
{
    ...
    for(i = y; *text; i += 7)
        BrPixelmapTextF(buffer, x - buffer->origin_x, i - buffer->origin_y, colour, BrFontProp4x6, *text++);
}
...
br_error BrDemo_InitApp(int argc, char **argv)
{
    ...
    r = BrDemo_Allocate_Pixelmaps("Fog Demo", &screen, &backscreen, &zbuffer, &palette, "fogdem.pal");
...
br_error BrDemo_Render_Loop()
{
    ...
    BrMatrix23PreRotate(&ground->material->map_transform, BR_ANGLE_DEG(BrScalarToFloat(BR_MUL(BR_SIN(a), BR_SCALAR(1)))));
    BrMaterialUpdate(ground->material, BR_MATU_MAP_TRANSFORM);

    BrMatrix34Translate(&ground->t.t.mat, 0, BR_SCALAR(-0.75), 0);
    BrMatrix34PostRotateZ(&ground->t.t.mat, BR_ANGLE_DEG(BrScalarToFloat(BR_MUL(BR_SCALAR(5), BR_COS(a)))));
