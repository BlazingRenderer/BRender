...
#undef _MAX
    ...
    size_t size;
    if((vertex_source = BrFileLoad(NULL, vertPath, &size)) == NULL)
        return BR_FALSE;

    vert = VIDEOI_CreateAndCompileShader(GL_VERTEX_SHADER, vertex_source, size);
    if(!vert) {
        BrResFree(vertex_source);
        return BR_FALSE;
    }

    BrResFree(vertex_source);

    if((fragment_source = BrFileLoad(NULL, fragPath, &size)) == NULL) {
        glDeleteShader(vert);
        return BR_FALSE;
    }

    frag = VIDEOI_CreateAndCompileShader(GL_FRAGMENT_SHADER, fragment_source, size);
    if(!frag) {
        glDeleteShader(vert);
        BrResFree(fragment_source);
        return BR_FALSE;
    }
